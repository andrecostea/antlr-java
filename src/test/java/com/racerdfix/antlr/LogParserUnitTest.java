...
import com.racerdfix.antlr.log.LogListener;
import com.racerdfix.antlr.log.model.LogLevel;
import com.racerdfix.antlr.log.model.LogEntry;
...
public class LogParserUnitTest {
    ...
    public void whenLogContainsOneErrorLogEntry_thenOneErrorIsReturned() throws Exception {
        ...
        com.racerdfix.antlr.log.LogListener logWalker = new LogListener();
        walker.walk(logWalker, logParser.log());

        assertThat(logWalker.getEntries().size(), is(2));
        LogEntry error = logWalker.getEntries().get(1);
        MatcherAssert.assertThat(error.getLevel(), CoreMatchers.is(LogLevel.ERROR));
